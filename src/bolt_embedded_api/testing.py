...
class Testing:
    ...
    def create_testing_shopper_account(self, request: operations.CreateTestingShopperAccountRequest, security: operations.CreateTestingShopperAccountSecurity) -> operations.CreateTestingShopperAccountResponse:
        ...
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[operations.CreateTestingShopperAccount200ApplicationJSON])
                res.create_testing_shopper_account_200_application_json_object = out
            else:
    ...
    def get_test_credit_card_token(self, security: operations.GetTestCreditCardTokenSecurity) -> operations.GetTestCreditCardTokenResponse:
        ...
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[operations.GetTestCreditCardToken200ApplicationJSON])
                res.get_test_credit_card_token_200_application_json_object = out
            else:
                raise errors.SDKError(f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
